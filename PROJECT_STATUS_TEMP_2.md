...
Still outstanding (not part of the original sprint scope, but noted as pending):

TC-23 / TC-24 - full E2E integration tests (need browser-based manual testing)
CORS config - frontend origin not explicitly whitelisted (causing the CORS error logs we saw)
Input validation - no express-validator on endpoints yet
Student favourites / visited history - frontend not wired
Image upload widget - not integrated
Postman collection - not exported
Delete-user endpoint - intentionally omitted (no UI button)
